...
def fetch_and_clean_images(query, index):
    ...
    valid_paths = []
    # FIX: Ensure we don't get .jpg.jpg
    for i, f in enumerate(os.listdir(raw_dir)):
        ...
        try:
            with Image.open(raw_path) as img:
                # Use simple naming to avoid extension stacking
...
def apply_ken_burns(clip):
    """Cinematic slow zoom effect (100% to 110%)."""
...
def generate_video(json_path):
    ...
    for i, item in enumerate(items):
        ...
        print(f"\n🎬 Rendering {i+1}/{len(items)} | {v_title}")
        ...
        if not img_paths:
            ...
            slides = []
            for p in img_paths:
                # Resize width to hit edges, then height to fill the gap
        ...
        # 5. Last Scene CTA (Strict Requirement)
        ...
        try:
            ...
            print(f"⚠️ Upload skipped/failed: {e}")

        gc.collect()

if __name__ == "__main__":
    # EXPLORER START: Select JSON
